from __future__ import annotations

import hashlib
import json
from dataclasses import dataclass
from typing import Any, Iterable, Sequence

from crypto_trading_mcp.execution.planner import TradePlan
from crypto_trading_mcp.paper.engine import PaperTradingEngine


@dataclass(frozen=True)
class Candle:
    timestamp: str
    open: float
    high: float
    low: float
    close: float
    volume: float
    symbol: str = "BTC/USD"

    def to_dict(self) -> dict[str, Any]:
        return {
            "timestamp": self.timestamp,
            "open": self.open,
            "high": self.high,
            "low": self.low,
            "close": self.close,
            "volume": self.volume,
            "symbol": self.symbol,
        }


class MarketDataReplay:
    """Feed OHLCV candles/ticks one-at-a-time into the paper engine."""

    def __init__(self, engine: PaperTradingEngine | None = None) -> None:
        self.engine = engine or PaperTradingEngine()
        self.processed: list[dict[str, Any]] = []

    def feed_candle(self, candle: Candle | dict[str, Any]) -> list[dict[str, Any]]:
        data = candle.to_dict() if isinstance(candle, Candle) else dict(candle)
        symbol = str(data.get("symbol", "BTC/USD")).upper()
        # Deterministic path: update on high then low then close so stops see extremes.
        events: list[dict[str, Any]] = []
        for key in ("open", "high", "low", "close"):
            px = float(data[key])
            self.engine.exchange.on_price_update(symbol, px)
            events.append({"symbol": symbol, "price": px, "source": key})
        self.processed.append(data)
        return events

    def feed_many(self, candles: Iterable[Candle | dict[str, Any]]) -> None:
        for candle in candles:
            self.feed_candle(candle)


class DeterministicReplay:
    """Replay identical paper runs from a frozen event tape."""

    def run(
        self,
        *,
        initial_cash: float,
        prices: list[tuple[str, float]],
        plans: list[TradePlan],
        seed_meta: dict[str, Any] | None = None,
        candles: Sequence[Candle | dict[str, Any]] | None = None,
    ) -> dict[str, Any]:
        engine = PaperTradingEngine()
        engine.reset()
        engine.exchange.portfolio = engine.exchange.portfolio.__class__(starting_cash=initial_cash)
        engine.exchange.session_id = "PAPER_SESSION_REPLAY"
        engine.start()
        results = []
        for symbol, price in prices:
            engine.exchange.set_price(symbol, price)
            engine.exchange.on_price_update(symbol, price)
        if candles:
            MarketDataReplay(engine).feed_many(candles)
        for plan in plans:
            px = engine.exchange.prices.get(plan.symbol.upper(), plan.entry_price)
            results.append(
                engine.execute_approved_plan(
                    plan,
                    market_price=px,
                    asset_class="CRYPTO",
                    analysis={"consensus": {"decision": plan.side, "confidence": plan.confidence}},
                )
            )
        snap = engine.exchange.portfolio.snapshot(engine.exchange.prices)
        payload = {
            "seed_meta": seed_meta or {},
            "orders": [o.model_dump(mode="json") for o in engine.exchange.orders.values()],
            "fills": [f.model_dump(mode="json") for f in engine.exchange.fills],
            "positions": snap.to_dict(),
            "fees": snap.fees,
            "trades": engine.exchange.trades,
            "execution_results": results,
        }
        structural = {
            "fill_count": len(engine.exchange.fills),
            "order_count": len(engine.exchange.orders),
            "cash": round(snap.cash, 8),
            "equity": round(snap.equity, 8),
            "realized_pnl": round(snap.realized_pnl, 8),
            "fees": round(snap.fees, 8),
        }
        payload["structural"] = structural
        payload["structural_hash"] = hashlib.sha256(
            json.dumps(structural, sort_keys=True).encode()
        ).hexdigest()
        return payload
